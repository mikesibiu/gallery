...
const isDemoAdminPreviewReadRoute = (uri: string) =>
  DEMO_ADMIN_PREVIEW_READ_ROUTES.some((route) => route.test(uri));
```
...
describe('authenticate admin preview guard', () => {
  ...
  it('redirects unauthenticated users to login before admin preview checks', async () => {
    ...
    await expect(authenticate(new URL('https://gallery.test/admin/users?tab=all'), { admin: true })).rejects.toMatchObject({
      status: 307,
