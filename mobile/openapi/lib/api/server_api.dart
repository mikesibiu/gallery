...
class ServerApi {
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getMlHealthWithHttpInfo({ Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
  ...
  ///
  /// Reports whether the ML server is currently reachable and healthy for smart search.
  Future<ServerMlHealthResponseDto?> getMlHealth({ Future<void>? abortTrigger, }) async {
    final response = await getMlHealthWithHttpInfo(abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
